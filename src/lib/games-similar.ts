...
import { publishedAtSortKey } from '@/lib/game-meta';

function publishedOrZero(value?: Date): number {
  return publishedAtSortKey(value);
}
...
export function getSimilarGamesByGameEngine(
  current: CollectionEntry<'games'>,
  all: CollectionEntry<'games'>[],
  max = 3,
): CollectionEntry<'games'>[] {
  const engines = current.data.gameEngine;
  if (!engines.length) return [];

  return all
    .filter((g) => g.id !== current.id)
    .filter((g) => overlapCount(engines, g.data.gameEngine) > 0)
    .sort((a, b) => {
      const oa = overlapCount(engines, a.data.gameEngine);
      const ob = overlapCount(engines, b.data.gameEngine);
      if (ob !== oa) return ob - oa;
      if (b.data.communityScore !== a.data.communityScore) {
        return b.data.communityScore - a.data.communityScore;
      }
